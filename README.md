...
- `transport-gmail` — sends and receives envelopes via Gmail using the local `gcloud` CLI's OAuth token (no separate OAuth app to register). Address format: `gmail://<email-address>`. RFC822 bodies carry a human-readable preamble plus the base64 payload between `-----GLON ENVELOPE BEGIN/END-----` markers, so recipients without Glon see a useful note. Inbox polling is filtered to `[GLON]`-prefixed subjects, internally rate-limited to 30s with exponential backoff on errors, and processed messages get a `Glon/Processed` label so they aren't re-handled.
...
- `swap-email` — drives a full atomic swap end-to-end over email. User says "swap 5 FIG for 10 GOOTECK with bob@x.com" in any chat surface; the program builds and escrows the offer locally, exports the changes as a `ChangeBundle`, sends them via `/transport-gmail` with `content_type=glon/swap-offer`, and watches the inbox for the response. The receiver's flow is symmetric: incoming `glon/swap-offer` surfaces to the human via `/user-chat`, `/swap-email accept <id>` pays + settles + claims and replies with `glon/swap-response`, `/swap-email decline <id>` (or the 20-min approval-deadline expiring) replies with `glon/swap-decline`. Originator timeout default 48h with cancel-return on expiry; receiver-approval default 20min so a slow recipient causes the originator to find out within ~21 min instead of waiting two days. Per-swap state lives in the program object's `persisted_state` field and survives daemon restarts.
